...
function Providers(){
    ...
    useEffect(() => {
        if (searchQuery) {
            api.get(`/providers/search?input=${searchQuery}`)
    ...
    const filteredProviders = providers
                        .filter((p) =>
                          selectedTab === "enabled" ? p.enabled === true : p.enabled === false
                        );
    ...
    const handleAddSubmit = async (e) => {
        ...
        try {
            ...
            console.error(err);
            setError(err.response.data.message || t('error.saving_provider'));
        }
    };
            ...
            {showAddForm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-[color:var(--secondary)] rounded-xl shadow-lg p-6 w-1/2 max-h-[90vh] flex flex-col">
                        <h3 className="text-xl font-semibold mb-4">{t('providers.add_provider')}</h3>
                            ...
                            <form className="flex flex-col gap-4" onSubmit={handleAddSubmit}>
                                <div className="flex gap-2">
                                    <input 
                                        type="text"
                                        name="comName"
                                        placeholder={t('providers.com_name')}
                                    ...
                                    <input
                                        type="text"
                                        name="legalName"
                                        placeholder={t('providers.legal_name')}
                                        onChange={handleChange}
                                        className="p-2 w-full rounded-lg border bg-[color:var(--background)]"
                                        required
                                        />
                                ...
                                <div className="flex gap-2">
                                    <input
                                        type="text"
                                        name="vatNumber"
                                        placeholder={t('providers.vat_number')}
                                        onChange={handleChange}
                                        className="p-2 w-full rounded-lg border bg-[color:var(--background)]"
                                        required
                                        />
                                ...
                                <div className="flex gap-2">
                                    <input 
                                        type="text"
                                        name="defaultVat"
                                        placeholder={t('providers.default_vat')}
                                        onChange={handleChange}
                                        className="p-2 w-full rounded-lg border bg-[color:var(--background)]"
                                        />
                                    <input
                                        ...
                                        placeholder={t('providers.default_withholding')}
                                        onChange={handleChange}
                                        className="p-2 w-full rounded-lg border bg-[color:var(--background)]"
                                        required
                                        />
                                ...
                                <div className="flex gap-2">
                                    <input
                                        type="text"
                                        name="duedate"
                                        placeholder={t('providers.duedate')}
                                        onChange={handleChange}
            ...
            <div className="rounded-xl shadow-lg w-3/4 p-4 bg-[color:var(--secondary)]">
                <div className="w-full flex flex-row">
                    <div className="w-full flex justify-between items-center mb-2">
                        <h4 className="text-lg font-semibold mb-2 w-1/2">{t('providers.providers')}</h4>
                ...
                <div className="overflow-x-auto w-full">
                    ...
                    <thead>
                        ...
                        <th 
                            ...
                            {t('providers.com_name')} {sortConfig.key === "comName" && (sortConfig.direction === "asc" ? "▲" : "▼")}
                        </th>
                        <th 
                            className="w-1/8 py-3 text-left text-[color:var(--text)] font-medium"
                            onClick={() => handleSort("vatNumber")}>
                            {t('providers.vat_number')} {sortConfig.key === "vatNumber" && (sortConfig.direction === "asc" ? "▲" : "▼")}
                        </th>
                        <th 
                            className="w-1/4 py-3 text-left text-[color:var(--text)] font-medium"
                            onClick={() => handleSort("email")}>
                            {t('providers.email')} {sortConfig.key === "email" && (sortConfig.direction === "asc" ? "▲" : "▼")}
                        </th>
                        <th 
                            className="w-1/4 py-3 text-left text-[color:var(--text)] font-medium"
                            onClick={() => handleSort("web")}>
                            {t('providers.web')} {sortConfig.key === "web" && (sortConfig.direction === "asc" ? "▲" : "▼")}
                        </th>
                        </tr>
                    </thead>
                    <tbody className="divide-y [color:var(--divide)]">
                        ...
                        >
                            ...
                            <td className="w-1/4 py-2">{provider.email}</td>
                            <td className="w-1/4 py-2">{provider.web}</td>
                        </tr>
                        ))}
                    </tbody>
                    </table>
